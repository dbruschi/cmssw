...
G4WentzelVIModelForCVH::G4WentzelVIModelForCVH(G4bool comb, const G4String& nam)
    : G4VMscModel(nam), singleScatteringMode(false), isCombined(comb), useSecondMoment(false) {
...
void G4WentzelVIModelForCVH::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts) {
  // reset parameters
  SetupParticle(p);
  ...
  */
  ...
  if (useSecondMoment && IsMaster() && nullptr != table) {
    ...
    //           << table << G4endl;
    fSecondMoments = G4PhysicsTableHelper::PreparePhysicsTable(fSecondMoments);
    ...
    if (emin < emax) {
      std::size_t n = G4EmParameters::Instance()->NumberOfBinsPerDecade() * G4lrint(std::log10(emax / emin));
      if (n < 3) {
        n = 3;
      }
...
void G4WentzelVIModelForCVH::InitialiseLocal(const G4ParticleDefinition*, G4VEmModel* masterModel) {
  fSecondMoments = static_cast<G4WentzelVIModelForCVH*>(masterModel)->GetSecondMomentTable();
}
...
G4double G4WentzelVIModelForCVH::ComputeCrossSectionPerAtom(
    const G4ParticleDefinition* p, G4double kinEnergy, G4double Z, G4double, G4double cutEnergy, G4double) {
  G4double cross = 0.0;
  SetupParticle(p);
  ...
  if (nullptr == CurrentCouple()) {
    G4Exception("G4WentzelVIModelForCVH::ComputeCrossSectionPerAtom",
                "em0011",
                FatalException,
...
G4double G4WentzelVIModelForCVH::ComputeTruePathLengthLimit(const G4Track& track, G4double& currentMinimalStep) {
  ...
  */
  ...
  G4double rlimit = std::max(facrange * currentRange, (1.0 - cosTetMaxNuc) * lambdaeff * invssFactor);
  ...
  if (rcut > rlimit) {
    rlimit = std::min(rlimit, rcut * sqrt(rlimit / rcut));
  }
...
G4ThreeVector& G4WentzelVIModelForCVH::SampleScattering(const G4ThreeVector& oldDirection, G4double /*safety*/) {
  ...
  */
  do {
    ...
    if (singleScat) {
      ...
      G4double cosTetM = wokvi->SetupTarget((*theElementVector)[i]->GetZasInt(), cut);
      ...
      G4bool isFirst = true;
      if (prob2 > 0.0 && rndmEngine->flat() < prob2) {
        isFirst = false;
      }
      do {
        //z = -z0*G4Log(1.0 - (1.0 - zzz)*rndmEngine->flat());
        if (isFirst) {
          z = -G4Log(rndmEngine->flat());
        } else {
          z = G4RandGamma::shoot(rndmEngine, 2.0, 2.0);
        }
        z *= z0;
...
G4ThreeVector G4WentzelVIModelForCVH::SampleScatteringTest(const G4ThreeVector& oldDirection, G4double /*safety*/) {
...
G4double G4WentzelVIModelForCVH::ComputeTransportXSectionPerVolume(G4double cosTheta) {
  ...
  for (G4int i = 0; i < nelm; ++i) {
    G4double costm = wokvi->SetupTarget((*theElementVector)[i]->GetZasInt(), cut);
...
G4double G4WentzelVIModelForCVH::ComputeSecondMoment(const G4ParticleDefinition* p, G4double kinEnergy) {
  G4double xs = 0.0;
  ...
  for (std::size_t i = 0; i < nelm; ++i) {
    G4double costm = wokvi->SetupTarget((*theElementVector)[i]->GetZasInt(), cut);
    xs += theAtomNumDensityVector[i] * wokvi->ComputeSecondTransportMoment(costm);
  }
  return xs;
}
