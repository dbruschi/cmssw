...
#ifdef G4VERBOSE
    if (G4ErrorPropagatorData::verbose() >= 3) {
      G4cout << " G4ErrorMessengerForCVH::MagFieldAction SetStepLimit " << MagFieldLimitCmd->GetNewDoubleValue(newValue)
             << G4endl;
    }
#endif
...
#ifdef G4VERBOSE
    if (G4ErrorPropagatorData::verbose() >= 3) {
      G4cout << " G4ErrorMessengerForCVH::EnergyLossAction SetStepLimit " << EnergyLossCmd->GetNewDoubleValue(newValue)
             << G4endl;
    }
#endif
