...
public:
  ...
  ~ResidualGlobalCorrectionMakerG4e() override {}
...
void ResidualGlobalCorrectionMakerG4e::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (unsigned int itrack = 0; itrack < trackOrigH->size(); ++itrack) {
    ...
    const edm::Ref<std::vector<pat::Muon>> muonref =
        doMuonAssoc_ ? (*muonAssoc)[trackref] : edm::Ref<std::vector<pat::Muon>>();
    ...
    Map<Matrix<float, 5, 5, RowMajor>>(trackCov.data()).triangularView<Upper>() =
        Map<const Matrix<double, 5, 5, RowMajor>>(tkcov.Array()).cast<float>().triangularView<Upper>();
      ...
      for (auto g = genPartCollection->begin(); g != genPartCollection->end(); ++g) {
        ...
        if (dR < drmin) {
          ...
          genParms[4] = (vtx.z() - myBeamSpot.z()) * g->pt() / g->p() -
                        ((vtx.x() - myBeamSpot.x()) * g->px() + (vtx.y() - myBeamSpot.y()) * g->py()) / g->pt() *
                            g->pz() / g->p();
        } else {
          continue;
        }
    ...
    for (auto it = track.recHitsBegin(); it != track.recHitsEnd(); ++it) {
      ...
      if (detglued != nullptr && !(*it)->isValid()) {
        ...
        if (!hits.empty()) {
          const bool duplicate = detinner->geographicalId() == hits.back()->geographicalId() ||
                                 detouter->geographicalId() == hits.back()->geographicalId();
        ...
        if (hitquality) {
          ...
          hits.push_back(
              TrackingRecHit::RecHitPointer(new InvalidTrackingRecHit(*detectorG, TrackingRecHit::inactive)));
        }
      }
    }
    ...
    for (unsigned int iiter = 0; iiter < niters; ++iiter) {
      ...
      Matrix<double, 5, 5> ref2curvjac =
          dopca ? pca2curvJacobianD(refFts, field, *bsH) : Matrix<double, 5, 5>::Identity();
      ...
      for (unsigned int ihit = 0; ihit < hits.size(); ++ihit) {
        ...
        auto const &propresult =
            simhitdebug
                ? g4prop->propagateGenericWithJacobianAltD(propfromtsos, surface, dbetaval, dxival, dmsval, dionival)
                : g4prop->propagateGenericWithJacobianAltD(updtsos, surface, dbetaval, dxival, dmsval, dionival);
        ...
        const float enext =
            simhit == nullptr ? -99. : std::sqrt(std::pow(simhit->pabs(), 2) + mmu * mmu) - 0.5 * simhit->energyLoss();
        ...
        if (dolocalupdate) {
          if (iiter == 0) {
            ...
            const Matrix<double, 5, 5> Hold =
                curv2localJacobianAltelossD(oldtsos, field, surface, dEdxlast, mmu, dbetaval);
        ...
        if (fitFromSimParms_) {
          ...
          auto const &propresultsalign =
              g4prop->propagateGenericWithJacobianAltD(simtsos, surfacealign, dbetaval, dxival, dmsval, dionival);
...

        {
          ...
          if (dolocalupdate) {
            ...
            Ffull.block<nlocalcons, nlocalstateparms>(icons, fullstateidx + nlocalstateparms) =
                Matrix<double, nlocalcons, nlocalstateparms>::Identity();
...

          {
            ...
            if (hit1d) {
              ...
              if (ispixel) {
                ...
                const ProxyStripTopology *proxytopology =
                    dynamic_cast<const ProxyStripTopology *>(&(preciseHit->det()->topology()));

                const TkRadialStripTopology *radialtopology =
                    dynamic_cast<const TkRadialStripTopology *>(&proxytopology->specificTopology());
            ...
            const unsigned int yresglobalidx =
                dores && ispixel ? detidparms.at(std::make_pair(9, hit->geographicalId())) : 0;
            ...
            if (iiter == 0) {
              ...
              if (doSim_) {
                if (simhit != nullptr) {
                  ...
                  dy0simgenlocal << simhit->localPosition().x() - localxval, simhit->localPosition().y() - localyval;
      ...
      Map<Matrix<float, 5, 5, RowMajor>>(refCov.data()).triangularView<Upper>() =
          (Cinner).cast<float>().triangularView<Upper>();
      ...
      if (std::isnan(edmval) || std::isinf(edmval)) {
        std::cout << "WARNING: invalid parameter update!!!"
                  << " edmval = " << edmval << " lamupd = " << lamupd << " deltachisqval = " << deltachisqval
                  << std::endl;
        valid = false;
        break;
      }
      ...
      if (anomDebug) {
        std::cout << "anomDebug: iiter = " << iiter << " edmval = " << edmval << " deltachisqval = " << deltachisqval
                  << " chisqval = " << chisqval << std::endl;
      }
  ...
  if (doMuonAssoc_) {
    ...
    corPtMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(corPtV.begin()), std::make_move_iterator(corPtV.end()));
    corPtMapFiller.fill();

    edm::ValueMap<float>::Filler corEtaMapFiller(corEtaMap);
    corEtaMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(corEtaV.begin()), std::make_move_iterator(corEtaV.end()));
    corEtaMapFiller.fill();

    edm::ValueMap<float>::Filler corPhiMapFiller(corPhiMap);
    corPhiMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(corPhiV.begin()), std::make_move_iterator(corPhiV.end()));
    corPhiMapFiller.fill();

    edm::ValueMap<int>::Filler corChargeMapFiller(corChargeMap);
    corChargeMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(corChargeV.begin()), std::make_move_iterator(corChargeV.end()));
    corChargeMapFiller.fill();

    edm::ValueMap<float>::Filler edmvalMapFiller(edmvalMap);
    edmvalMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(edmvalV.begin()), std::make_move_iterator(edmvalV.end()));
    edmvalMapFiller.fill();

    edm::ValueMap<int>::Filler nValidHitsMapFiller(nValidHitsMap);
    nValidHitsMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(nValidHitsV.begin()), std::make_move_iterator(nValidHitsV.end()));
    nValidHitsMapFiller.fill();

    edm::ValueMap<int>::Filler nValidPixelHitsMapFiller(nValidPixelHitsMap);
    nValidPixelHitsMapFiller.insert(muonAssoc->ref(),
                                    std::make_move_iterator(nValidPixelHitsV.begin()),
                                    std::make_move_iterator(nValidPixelHitsV.end()));
    nValidPixelHitsMapFiller.fill();

    edm::ValueMap<std::vector<int>>::Filler globalidxsMapFiller(globalidxsMap);
    globalidxsMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(globalidxsV.begin()), std::make_move_iterator(globalidxsV.end()));
    globalidxsMapFiller.fill();

    edm::ValueMap<std::vector<float>>::Filler jacRefMapFiller(jacRefMap);
    jacRefMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(jacRefV.begin()), std::make_move_iterator(jacRefV.end()));
    jacRefMapFiller.fill();

    edm::ValueMap<std::vector<float>>::Filler momCovMapFiller(momCovMap);
    momCovMapFiller.insert(
        muonAssoc->ref(), std::make_move_iterator(momCovV.begin()), std::make_move_iterator(momCovV.end()));
