...
void ResidualGlobalCorrectionMakerBase::beginRun(edm::Run const &run, edm::EventSetup const &es) {
  edm::ESHandle<GlobalTrackingGeometry> globalGeometryNominal = es.getHandle(globalGeometryNominalToken_);

  edm::ESHandle<TrackerGeometry> globalGeometryIdeal = es.getHandle(globalGeometryIdealToken_);

  const TrackingGeometry *globalGeometry = useIdealGeometry_
                                               ? static_cast<const TrackingGeometry *>(globalGeometryIdeal.product())
                                               : static_cast<const TrackingGeometry *>(globalGeometryNominal.product());
  ...
  if (dofieldtest) {
    ...
    for (int ipoint = 0; ipoint < npoints; ++ipoint) {
      ...
      if (!imatch) {
        std::cout << "field mismatch! ipoint = " << ipoint << " fwd: " << ifieldfwd << " rev: " << ifieldrev
                  << std::endl;
        fieldmatch = false;
      }
    }
  ...
  if (detidparms.empty()) {
    ...
    for (const auto &key : parmset) {
      ...
      if (true) {
        ...
        const GlobalVector posv(
            det->surface().position().x(), det->surface().position().y(), det->surface().position().z());
    ...
    if (GeomDetEnumerators::isTracker(det->subDetector())) {
      ...
      if (isglued) {
        ...
        if (alignGlued_) {
          ...
          const GloballyPositioned<double> surfaceIdeal =
              surfaceToDouble(surfaceIdealPre, zsign * surfaceGluedIdeal.rotation().z());
...
GloballyPositioned<double> ResidualGlobalCorrectionMakerBase::surfaceToDouble(const Surface &surface,
                                                                              const Basic3DVector<double> &gz) const {
  const Point3DBase<double, GlobalTag> pos = surface.position();
...
Matrix<double, 6, 1> ResidualGlobalCorrectionMakerBase::globalToLocal(const Matrix<double, 7, 1> &state,
                                                                      const GloballyPositioned<double> &surface) const {
...
Matrix<double, 7, 1> ResidualGlobalCorrectionMakerBase::localToGlobal(const Matrix<double, 6, 1> &localstate,
                                                                      const GloballyPositioned<double> &surface) const {
...
Matrix<double, 5, 6> ResidualGlobalCorrectionMakerBase::hybrid2curvJacobianD(const Matrix<double, 7, 1> &state,
...
Matrix<double, 7, 1> ResidualGlobalCorrectionMakerBase::pca2cart(const Matrix<double, 5, 1> &statepca,
...
Matrix<double, 5, 1> ResidualGlobalCorrectionMakerBase::cart2pca(const Matrix<double, 7, 1> &state,
                                                                 const reco::BeamSpot &bs) const {
  const double x0bs = bs.x0();
  const double y0bs = bs.y0();
  const double z0bs = bs.z0();
...
Matrix<double, 5, 5> ResidualGlobalCorrectionMakerBase::pca2curvJacobianD(const Matrix<double, 7, 1> &state,
                                                                          const MagneticField *field,
  ...
  const double xf25 = std::pow(-2 * xf16 * xf17 * xf4 - xf19 * xf8 + xf19 + xf20 * xf4 * xf7 - xf20 * xf4 + xf20 -
                                   2 * xf21 * xf22 - 2 * xf23 * xf24 + xf4,
                               -1.0 / 2.0);
  ...
  const double xf74 =
      -xf11 * xf41 + (1.0 / 2.0) * xf36 * (2 * xf70 + 2 * xf71) + (1.0 / 2.0) * xf40 * (2 * xf72 + 2 * xf73);
  ...
  const double dlamdlam0 =
      xf13 * xf27 + (xf0 * (xf11 * xf2 + xf11 * xf7) / std::pow(xf9, 3.0 / 2.0) + xf3 / std::sqrt(xf9)) /
                        (std::pow(xf0, 2) * xf10 + 1);
  ...
  const double dytdqop0 = W0x * W0z * xf49 * xf51 * xf52 * xf64 * xf65 + W0y * W0z * xf49 * xf51 * xf52 * xf64 * xf66 -
                          xf101 * xf102 * xf52 * xf64 - xf44 * xf97 +
                          xf45 * xf51 * xf52 * xf98 * (xf100 + xf14 * xf99) - xf60 * xf95;
  const double dytdlam0 = d0 * xf103 * xf18 * xf25 * xf49 + xf101 * xf45 * xf52 * xf78 * xf79 * xf98 - xf104 * xf81 -
                          xf105 * xf81 + xf45 * xf51 * xf52 * xf98 * (B * qop0 * xf51 * xf74 * z0 - xf0 * xf15 * xf18) -
                          xf75 * xf97 - xf77 * xf95;
  const double dytdphi0 =
      d0 * xf103 * xf25 * xf28 * xf3 * xf49 + xf101 * xf45 * xf52 * xf79 * xf89 * xf98 - xf104 * xf90 - xf105 * xf90 +
      xf45 * xf51 * xf52 * xf98 * (xf35 * (xf84 + xf85) + xf76 * xf86 * z0) - xf87 * xf97 - xf88 * xf95;
...
Matrix<double, 6, 5> ResidualGlobalCorrectionMakerBase::pca2cartJacobianD(const Matrix<double, 7, 1> &state,
                                                                          const reco::BeamSpot &bs) const {
  const double dxdzbs = bs.dxdz();
  const double dydzbs = bs.dydz();
  ...
  const double dxdlam0 = xf17 * (std::pow(dydzbs, 3) * xf10 * xf4 - dydzbs * xf10 * xf4 * xf5 + dydzbs * xf10 * xf4 +
                                 dydzbs * xf4 * xf5 + xf0 * xf5 * xf7 - xf1 * xf3 - 2 * xf11 * xf9 - xf9);
  const double dxdphi0 = xf21 * (-xf14 * xf20 - xf16 * xf2);
  const double dxdd0 = xf14 * xf22;
  const double dxdz0 = dxdzbs;
  const double dydqop0 = 0;
  const double dydlam0 =
      xf17 * (-std::pow(dxdzbs, 3) * xf24 * xf4 + dxdzbs * dydzbs * xf0 * xf7 + dxdzbs * xf11 * xf24 * xf4 +
              2 * dydzbs * xf2 * xf4 * xf5 * xf7 + dydzbs * xf2 * xf4 * xf7 - xf11 * xf23 - xf2 * xf25 - xf23 * xf24);
...
std::array<Matrix<double, 7, 1>, 2> ResidualGlobalCorrectionMakerBase::twoTrackPca2cart(
    const Matrix<double, 10, 1> &statepca) const {
...
Matrix<double, 10, 1> ResidualGlobalCorrectionMakerBase::twoTrackCart2pca(const Matrix<double, 7, 1> &state0,
                                                                          const Matrix<double, 7, 1> &state1) const {
...
Matrix<double, 10, 10> ResidualGlobalCorrectionMakerBase::twoTrackPca2curvJacobianD(const Matrix<double, 7, 1> &state0,
                                                                                    const Matrix<double, 7, 1> &state1,
                                                                                    const MagneticField *field,
                                                                                    double dBz0,
                                                                                    double dBz1) const {
  ...
  const double dlamadlama0 =
      xf46 * xf80 + (xf1 * (xf47 * xf53 + xf49 * xf53) / std::pow(xf51, 3.0 / 2.0) + xf6 / std::sqrt(xf51)) /
                        (std::pow(xf1, 2) * xf52 + 1);
  ...
  const double dytadlama0 =
      xf128 * xf138 * xf80 - xf136 * xf73 - xf136 * xf76 - xf137 * xf78 - xf137 * xf79 + xf140 * xf68;
  const double dytadphia0 =
      xf128 * xf138 * xf94 - xf136 * xf88 - xf136 * xf91 - xf137 * xf92 - xf137 * xf93 + xf140 * xf87;
  const double dytadqopb0 = 0;
  const double dytadlamb0 =
      xf101 * xf140 - xf103 * xf136 - xf105 * xf136 - xf106 * xf137 - xf108 * xf137 + xf109 * xf128 * xf138;
  const double dytadphib0 =
      xf111 * xf140 - xf114 * xf136 - xf116 * xf136 - xf117 * xf137 - xf118 * xf137 + xf119 * xf128 * xf138;
  ...
  const double dlambdlamb0 =
      xf166 * xf195 + (xf2 / std::sqrt(xf186) + xf7 * (xf182 * xf188 + xf184 * xf188) / std::pow(xf186, 3.0 / 2.0)) /
                          (xf187 * std::pow(xf7, 2) + 1);
  ...
  const double dytbdlama0 =
      xf142 * xf217 - xf151 * xf215 - xf157 * xf215 - xf159 * xf216 - xf162 * xf216 + xf164 * xf209 * xf218;
  const double dytbdphia0 =
      xf167 * xf217 - xf169 * xf215 - xf172 * xf215 - xf173 * xf216 - xf174 * xf216 + xf175 * xf209 * xf218;
  const double dytbdqopb0 = -xf176 * xf215 + xf178 * xf215 - xf179 * xf216 + xf180 * xf216 + xf181 * xf219;
  const double dytbdlamb0 =
      xf189 * xf217 - xf191 * xf215 - xf192 * xf215 - xf193 * xf216 - xf194 * xf216 + xf195 * xf209 * xf218;
  const double dytbdphib0 =
      xf196 * xf217 - xf197 * xf215 - xf199 * xf215 - xf200 * xf216 - xf201 * xf216 + xf202 * xf209 * xf218;
...
Matrix<double, 5, 5> ResidualGlobalCorrectionMakerBase::curv2localJacobianAltelossD(
    const Matrix<double, 7, 1> &state,
    const MagneticField *field,
    const GloballyPositioned<double> &surface,
    double dEdx,
    double mass,
    double dBz) const {
  const GlobalPoint pos(state[0], state[1], state[2]);
  ...
  const double x23 =
      std::pow(Ix1 * std::cos(x20) + Ix1 * std::cos(x22) + Iy1 * std::sin(x20) - Iy1 * std::sin(x22) + 2 * x7, -2);
  ...
  const double ddxdzdphi0 = x23 * (x18 * x31 - x18 * x36 + x19 * x32 + x19 * x37 + x25 * x29 + x25 - x27 * x29 - x27 -
                                   x31 * x33 - x32 * x34 + x33 * x36 - x34 * x37);
  ...
  const double ddydzdphi0 = x23 * (x29 * x50 - x29 * x51 + x31 * x48 - x31 * x52 + x32 * x49 - x32 * x53 - x36 * x48 +
                                   x36 * x52 + x37 * x49 - x37 * x53 + x50 - x51);
...
Matrix<double, 5, 5> ResidualGlobalCorrectionMakerBase::curv2localhybridJacobianAltelossD(
    const Matrix<double, 7, 1> &state,
    const MagneticField *field,
    const GloballyPositioned<double> &surface,
    double dEdx,
...
Matrix<double, 5, 11> ResidualGlobalCorrectionMakerBase::curv2localJacobianAlignmentD(
    const Matrix<double, 7, 1> &state,
    const MagneticField *field,
    const GloballyPositioned<double> &surface,
    double dEdx,
...
Matrix<double, 2, 1> ResidualGlobalCorrectionMakerBase::localPositionConvolutionD(
    const Matrix<double, 7, 1> &state,
    const Matrix<double, 5, 5> &curvcov,
    const GloballyPositioned<double> &surface) const {
  ...
  const double d2vdlamdlam = x36 * x47 + x36 * x62 + x40 * x50 + x40 * x60 + x43 * x53 + x43 * x61 + x48 * x70 +
                             x51 * x70 + x54 * x67 + x63 * x64 + x65 * x66 + x70 * x71;
  const double d2vdlamdphi = x35 * x89 + x39 * x93 + x40 * x85 + x42 * x94 + x43 * x84 + x48 * x86 + x48 * x96 +
                             x51 * x86 + x51 * x96 + x54 * x90 + x63 * x83 + x65 * x92 + x71 * x86 + x71 * x96 +
                             x76 * x77;
  ...
  const double d2vdphidphi = x122 * x89 + x123 * x40 + x124 * x43 + x125 * x48 + x125 * x51 + x125 * x71 + x126 * x94 +
                             x127 * x93 + x128 * x48 + x128 * x51 + x128 * x71 + x55 * x77;
  ...
  const double d2wdlamdlam = x134 * x47 + x134 * x62 + x135 * x50 + x135 * x60 + x136 * x53 + x136 * x61 + x137 * x70 +
                             x138 * x66 + x139 * x67 + x140 * x64 + x141 * x70 + x142 * x70;
  const double d2wdlamdphi = x135 * x85 + x136 * x84 + x137 * x86 + x137 * x96 + x138 * x92 + x139 * x90 + x140 * x83 +
                             x141 * x86 + x141 * x96 + x142 * x86 + x142 * x96 + x143 * x76 + x144 * x35 + x145 * x39 +
                             x146 * x42;
  ...
  const double d2wdphidphi = x122 * x144 + x123 * x135 + x124 * x136 + x125 * x137 + x125 * x141 + x125 * x142 +
                             x126 * x146 + x127 * x145 + x128 * x137 + x128 * x141 + x128 * x142 + x143 * x55;
...
Matrix<double, 1, 6> ResidualGlobalCorrectionMakerBase::massJacobianAltD(const Matrix<double, 7, 1> &state0,
                                                                         const Matrix<double, 7, 1> &state1,
                                                                         double dmass) const {
...
Matrix<double, 6, 6> ResidualGlobalCorrectionMakerBase::massHessianAltD(const Matrix<double, 7, 1> &state0,
                                                                        const Matrix<double, 7, 1> &state1,
                                                                        double dmass) const {
  const double mp = dmass;
  ...
  const double d2mdqop0dqop0 = xf27 * (-xf29 * xf33 - xf34 * xf36 - xf34 * xf37 + 3 * xf25 * xf38 / std::pow(qop0, 4) -
                                       xf25 / (std::pow(qop0, 6) * std::pow(xf21, 3.0 / 2.0))) +
                               xf42 * xf44;
  ...
  const double d2mdqop1dqop1 = xf27 * (-xf29 * xf90 - xf36 * xf91 - xf37 * xf91 + 3 * xf22 * xf60 / std::pow(qop1, 4) -
                                       xf22 / (std::pow(qop1, 6) * std::pow(xf24, 3.0 / 2.0))) +
                               xf65 * xf89;
...
Matrix<double, 1, 6> ResidualGlobalCorrectionMakerBase::massinvsqJacobianAltD(const Matrix<double, 7, 1> &state0,
                                                                              const Matrix<double, 7, 1> &state1,
                                                                              double dmass) const {
  const double mp = dmass;
...
Matrix<double, 6, 6> ResidualGlobalCorrectionMakerBase::massinvsqHessianAltD(const Matrix<double, 7, 1> &state0,
                                                                             const Matrix<double, 7, 1> &state1,
                                                                             double dmass) const {
  const double mp = dmass;
  ...
  const double d2minvsqdqop0dqop0 = xf38 * (xf14 * xf9 + xf18 * xf9 + xf2 * xf7 - 6 * xf24 * xf26 / std::pow(qop0, 4) +
                                            2 * xf24 / (std::pow(qop0, 6) * std::pow(xf21, 3.0 / 2.0))) +
                                    xf44 * xf48;
  ...
  const double d2minvsqdqop1dqop1 =
      xf38 * (xf100 * xf2 + xf101 * xf14 + xf101 * xf18 - 6 * xf25 * xf65 / std::pow(qop1, 4) +
              2 * xf25 / (std::pow(qop1, 6) * std::pow(xf23, 3.0 / 2.0))) +
      xf72 * xf99;
