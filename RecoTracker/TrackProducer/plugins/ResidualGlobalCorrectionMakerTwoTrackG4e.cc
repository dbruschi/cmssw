...
public:
  ...
  ~ResidualGlobalCorrectionMakerTwoTrackG4e() override {}
...
void ResidualGlobalCorrectionMakerTwoTrackG4e::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (auto itrack = trackOrigH->begin(); itrack != trackOrigH->end(); ++itrack) {
    ...
    for (auto jtrack = itrack + 1; jtrack != trackOrigH->end(); ++jtrack) {
      ...
      const std::array<bool, 2> highpurityarr = {
          {itrack->quality(reco::TrackBase::highPurity), jtrack->quality(reco::TrackBase::highPurity)}};
      ...
      for (unsigned int icons = 0; icons < nicons; ++icons) {
        ...
        for (unsigned int iiter = 0; iiter < niters; ++iiter) {
          ...
          const Matrix<double, 10, 10> twotrackpca2curvref =
              twoTrackPca2curvJacobianD(refftsarr[0], refftsarr[1], field, dbetavalrefarr[0], dbetavalrefarr[1]);
          ...
          for (unsigned int id = 0; id < 2; ++id) {
            ...
            if (bsConstraint_) {
              ...
              hessfull.block<nlocalvtx, nlocalvtx>(fullvtxidx, fullvtxidx) +=
                  hesslocal.topLeftCorner<nlocalvtx, nlocalvtx>();
            }
            ...
            for (unsigned int ihit = 0; ihit < hits.size(); ++ihit) {
              ...
              const Matrix<double, 5, 5> Hm =
                  curv2localJacobianAltelossD(updtsos, field, surface, dEdxlast, mmu, dbetaval);
              ...
              if (ihit == 0) {
                ...
                if (dolocalupdate) {
                  Fprop.middleCols<nvtxstate>(localvtxidx) =
                      -Hm * FdFm.leftCols<5>() * twotrackpca2curvref.middleRows<5>(vtxjacidx);
                  Fprop.middleCols<nlocalstate>(localstateidx) = Hp;
                  Fprop.middleCols<nlocalparms>(localparmidx) = -Hm * FdFm.rightCols<2>();
                } else {
                  Fprop.middleCols<nvtxstate>(localvtxidx) =
                      -FdFm.leftCols<5>() * twotrackpca2curvref.middleRows<5>(vtxjacidx);
                ...
                for (unsigned int iidx = 0; iidx < localidxs.size(); ++iidx) {
                  gradfull.segment(fullidxs[iidx], localsizes[iidx]) +=
                      propgrad.segment(localidxs[iidx], localsizes[iidx]);
                  for (unsigned int jidx = 0; jidx < localidxs.size(); ++jidx) {
                    hessfull.block(fullidxs[iidx], fullidxs[jidx], localsizes[iidx], localsizes[jidx]) +=
                        prophess.block(localidxs[iidx], localidxs[jidx], localsizes[iidx], localsizes[jidx]);
                  }
                }
              } else {
              ...
              if (hit->isValid()) {
                ...
                LocalTrajectoryParameters locparm(
                    localparms[0], localparms[1], localparms[2], localparms[3], localparms[4], localparms[5]);
                ...
                auto fillAlignGrads = [&](auto Nalign) {
                  ...
                  for (unsigned int iidx = 0; iidx < localidxs.size(); ++iidx) {
                    gradfull.segment(fullidxs[iidx], localsizes[iidx]) +=
                        hitgrad.segment(localidxs[iidx], localsizes[iidx]);
                    for (unsigned int jidx = 0; jidx < localidxs.size(); ++jidx) {
                      hessfull.block(fullidxs[iidx], fullidxs[jidx], localsizes[iidx], localsizes[jidx]) +=
                          hithess.block(localidxs[iidx], localidxs[jidx], localsizes[iidx], localsizes[jidx]);
                    }
                  }
          ...
          if (icons > 0) {
            ...
            const ROOT::Math::PxPyPzMVector mom0(refFts0[3], refFts0[4], refFts0[5], mmu);

            const ROOT::Math::PxPyPzMVector mom1(refFts1[3], refFts1[4], refFts1[5], mmu);
            ...
            for (unsigned int iidx = 0; iidx < localidxs.size(); ++iidx) {
              gradfull.segment(fullidxs[iidx], localsizes[iidx]) +=
                  gradlocal.segment(localidxs[iidx], localsizes[iidx]);
              for (unsigned int jidx = 0; jidx < localidxs.size(); ++jidx) {
                hessfull.block(fullidxs[iidx], fullidxs[jidx], localsizes[iidx], localsizes[jidx]) +=
                    hesslocal.block(localidxs[iidx], localidxs[jidx], localsizes[iidx], localsizes[jidx]);
              }
            }
          }
          ...
          const Matrix<double, 1, 1> deltachisq =
              dchisqdx.transpose() * dxfull + 0.5 * dxfull.transpose() * d2chisqdx2 * dxfull;
          ...
          if (muplusgen != nullptr && muminusgen != nullptr) {
            auto const jpsigen =
                ROOT::Math::PtEtaPhiMVector(muplusgen->pt(), muplusgen->eta(), muplusgen->phi(), mmu) +
          ...
          if (std::isnan(edmval) || std::isinf(edmval)) {
            std::cout << "WARNING: invalid parameter update!!!"
                      << " edmval = " << edmval << " deltachisqval = " << deltachisqval << std::endl;
            valid = false;
            break;
          }
        ...
        if (icons == 0) {
          ...
          Map<Matrix<float, 3, Dynamic, RowMajor>>(Muplus_jacRef.data(), 3, nparsfinal) =
              dxdparms.block(0, trackstateidxplus, nparsfinal, 3).transpose().cast<float>();

          Muminus_jacRef.resize(3 * nparsfinal);
          Map<Matrix<float, 3, Dynamic, RowMajor>>(Muminus_jacRef.data(), 3, nparsfinal) =
              dxdparms.block(0, trackstateidxminus, nparsfinal, 3).transpose().cast<float>();

          const Matrix<double, 1, 6> mjacalt = massJacobianAltD(refftsarr[0], refftsarr[1], mmu);

          Jpsi_jacMass.resize(nparsfinal);
          Map<Matrix<float, 1, Dynamic, RowMajor>>(Jpsi_jacMass.data(), 1, nparsfinal) =
              (mjacalt * dxdparms.leftCols<6>().transpose()).cast<float>();
        }
      }
